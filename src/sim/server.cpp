...
//  std::foreach(plugins.begin(), plugins.end(), [](auto &plugin) { plugin.state = robot_state_; })
}

void Server::PublishWorldStats(double rtf) {
  ssim::WorldStatistics world_stats_msg;
  world_stats_msg.step = steps_;
  world_stats_msg.sim_time = sim_time_;
  world_stats_msg.real_time_factor = rtf;

//  std::foreach(plugins.begin(), plugins.end(), [](auto &plugin) { plugin.world_stats = world_stats; })
}

void Server::OnServerControl(const ssim::ServerControl &msg) {
//  if (msg.has_pause()) {
//    pause_ = msg.pause();
//  } else if (msg.toggle_play_pause()) {
//    ssim::ServerControl play_pause_msg;
...
void Server::OnPhysics(const ssim::PhysicsConfig &msg) {
...
void Server::OnMaze(const ssim::Maze &msg) {
//  ssim::Convert(msg, maze_walls_);
}

void Server::OnRobotCommand(const ssim::RobotCommand &msg) {
  cmd_ = msg;
}

void Server::OnRobotDescription(const ssim::RobotDescription &msg) {
  mouse_ = msg;
...
int Server::ComputeADCValue(ssim::SensorDescription sensor) {
...
double Server::ComputeSensorDistToWall(ssim::SensorDescription sensor) {
...
//  }
//
//  if (min_range < ssim::ANALOG_MIN_DIST_CU) {
//    min_range = ssim::ANALOG_MIN_DIST_CU;
//  }
//
//  return ssim::toMeters(min_range);
}
...
//
//  max_cells_to_check_ = (unsigned int) std::ceil(ssim::toCellUnits(max_range));
}

double Server::ComputeSensorRange(const ssim::SensorDescription sensor) {
//  double sensor_x = sensor.p().x();
//  double sensor_y = sensor.p().x();
//  double range_x = sensor_x + cos(sensor.p().theta()) * ssim::ANALOG_MAX_DIST_M;
//  double range_y = sensor_y + sin(sensor.p().theta()) * ssim::ANALOG_MAX_DIST_M;
