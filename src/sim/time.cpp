...
const Time Time::Second = Time(1, 0);
const Time Time::Hour = Time(3600, 0);
...
/////////////////////////////////////////////////
Time::Time(const Time &_time) noexcept
    : sec(_time.sec), nsec(_time.nsec) {
}

/////////////////////////////////////////////////
Time::Time(const struct timeval &_tv) noexcept {
  this->sec = _tv.tv_sec;
  this->nsec = _tv.tv_usec * 1000;
}

/////////////////////////////////////////////////
Time::Time(const struct timespec &_tv) noexcept {
  this->sec = _tv.tv_sec;
  this->nsec = _tv.tv_nsec;
}

/////////////////////////////////////////////////
Time::Time(int32_t _sec, int32_t _nsec) noexcept
    : sec(_sec), nsec(_nsec) {
  this->Correct();
}

/////////////////////////////////////////////////
Time::Time(double _time) noexcept {
  this->Set(_time);
}
...
const Time &Time::GetWallTime() {
  ...
  // Normalize
  {
    ...
    if (secPart < 0 || secPart > UINT_MAX)
      std::err << "Time is out of dual 32-bit range\n";
...
Time Time::Sleep(const Time &_time) {
  ...
  if (_time >= clockResolution) {
    ...
    if (timer == nullptr)
    {
      std::cerr << "Unable to create waitable timer. Sleep will be incorrect.\n";
      return result;
    }

    if (!SetWaitableTimer (timer, &sleepTime, 0, nullptr, nullptr, 0))
    {
      std::cerr << "Unable to use waitable timer. Sleep will be incorrect.\n";
      return result;
    }

    if (WaitForSingleObject (timer, INFINITE) != WAIT_OBJECT_0)
    {
      std::cerr << "Unable to wait for a single object. Sleep will be incorrect.\n";
      return result;
    }
