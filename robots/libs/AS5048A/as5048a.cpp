 ...
 */
unsigned int AS5048A::getUnsignedRotation() {
  word data;
  int rotation;

  data = AS5048A::getRawRotation();
  rotation = (int) data - (int) position;
  if (rotation < 0) rotation = 0x3FFF + rotation;
 ...
 */
void AS5048A::ResetPosition() {
  word current_position = getRawRotation();
  position = current_position % 0x3FFF;
}
